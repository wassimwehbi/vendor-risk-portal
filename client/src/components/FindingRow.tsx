  ...
  return (
    <>
      <tr className="align-top hover:bg-slate-50/60">
        ...
        <td className="px-3 py-3">
          <span
            className={`mb-1 inline-flex rounded px-1.5 py-0.5 text-[11px] font-medium ${RESPONSE_TYPE_CLASSES[item.response_type] ?? 'bg-slate-100'}`}
          >
            {item.response_type}
          </span>
        ...
        <td className="px-3 py-3">
          <span
            className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium ring-1 ring-inset ${EVIDENCE_CLASSES[eff.evidence_sufficiency]}`}
          >
          ...
          <td colSpan={8} className="px-4 py-4">
            <div className="grid gap-4 lg:grid-cols-2">
              <div className="space-y-2 break-words text-sm">
                <p>
                  <span className="font-medium text-slate-600">Question:</span> {item.question_text}
                </p>
                {item.vendor_comments && (
                  <p>
                    <span className="font-medium text-slate-600">Vendor comments:</span> {item.vendor_comments}
                  </p>
                )}
                <p>
                  <span className="font-medium text-slate-600">Evidence provided:</span>{' '}
                  {item.evidence_text || '(none)'} {item.evidence_location ? `— ${item.evidence_location}` : ''}
                </p>
                {item.expiration_date && (
                  <p>
                    <span className="font-medium text-slate-600">Expiration:</span> {item.expiration_date}
                  </p>
                )}
                <p className="text-slate-500">
                  <span className="font-medium text-slate-600">AI rationale:</span> {finding.ai_rationale}
                </p>
                <div>
                  <p className="font-medium text-slate-600">Follow-up questions:</p>
                  <ul className="ml-4 list-disc text-slate-600">
                    {eff.follow_up_questions.length === 0 ? (
                      <li className="text-slate-500">None</li>
                    ) : (
                      eff.follow_up_questions.map((q, i) => <li key={i}>{q}</li>)
                    )}
                  </ul>
                </div>
              </div>
              {editing && canEdit && (
                <div className="space-y-3 rounded-lg border border-slate-200 bg-white p-4">
                  ...
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <div>
                      <label htmlFor={`${fid}-domain`} className="label text-xs">
                        Control area
                      </label>
                      <input
                        id={`${fid}-domain`}
                    ...
                    <div>
                      <label htmlFor={`${fid}-risk`} className="label text-xs">
                        Risk level
                      </label>
                      <select
                        id={`${fid}-risk`}
                  ...
                  <div>
                    <label htmlFor={`${fid}-evidence`} className="label text-xs">
                    ...
                    <select
                      ...
                      className="input"
                      value={evidence}
                      onChange={(e) => setEvidence(e.target.value as EvidenceSufficiency)}
                    >
                  ...
                  <div>
                    <label htmlFor={`${fid}-mappings`} className="label text-xs">
                      Framework mapping (one per line — <code>Framework: ref1; ref2</code>)
                    </label>
                    <textarea
                      id={`${fid}-mappings`}
                  ...
                  <div>
                    <label htmlFor={`${fid}-followups`} className="label text-xs">
