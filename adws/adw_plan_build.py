...
# ///
"""ADW Plan+Build (isolated). Usage: uv run adws/adw_plan_build.py <issue> [adw-id]"""
...
def main():
    if len(sys.argv) < 2:
        print("Usage: uv run adws/adw_plan_build.py <issue-number> [adw-id]")
        sys.exit(1)
    ...
    sys.exit(
        run_pipeline(
            issue_number,
            adw_id,
            [("adw_plan.py", [], True), ("adw_build.py", [], True)],
        )
    )
